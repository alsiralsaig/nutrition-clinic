...
router.get('/me', wrap(async (req, res) => {
  ...
  res.json({
    ...
    plan: plan && { id: plan.id, title: plan.title, target_kcal: plan.target_kcal, instructions: plan.instructions, totals: plan.totals, weekly: plan.weekly,
      meals: plan.meals.map((m) => ({ id: m.id, slot: m.slot, slot_time: m.slot_time, title: m.title, items: m.items, portions: m.portions, kcal: m.kcal, day_of_week: m.day_of_week })),
