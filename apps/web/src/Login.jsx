        ...
        <form onSubmit={submit} className="card card-pad w-full max-w-[400px] p-6 sm:p-8">
          ...
          {!installed && (
            <div className="mb-5">
              ...
              }} className="btn-soft w-full !py-2.5" data-install>
                {canPromptInstall() ? '⬇️ تثبيت التطبيق على الشاشة الرئيسية' : '📲 كيف أثبّت التطبيق على الشاشة الرئيسية؟'}
              </button>
              {showInstallHelp && (
                <ol className="mt-2 grid list-decimal gap-1 rounded-xl bg-sand p-3 ps-7 text-[12px] font-bold leading-6 text-ink/70" data-install-steps={os}>
                  {os === 'ios' ? <>
          ...
          {installed && <p className="mb-4 text-center text-[12px] font-extrabold text-brand-700">✓ التطبيق مثبت على هذا الجهاز</p>}
